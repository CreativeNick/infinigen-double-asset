...
# This particular Python script/file derived from `generate_individual_assets.py`, edited by:
# - Nicholas Jean
...
def build_scene_surface(factory_name, idx):
    try:
        with gin.unlock_config():
            ...
            material.node_tree.nodes['Principled BSDF'].inputs[0].default_value = .015, .009, .003, 0
            material.node_tree.nodes['Principled BSDF'].inputs['Alpha'].default_value = 0  # Set alpha to 0
...
def build_scene(path, idx, factory_name, args, chosen=False):
    ...
    cam_dist = np.random.uniform(args.cam_dist_min, args.cam_dist_max)
    cam_angle = np.random.uniform(args.cam_angle_min, args.cam_angle_max, size=3)
    camera, center = setup_camera(args, cam_dist, cam_angle)
    ...
    sky_texture.sun_rotation = np.random.uniform(0, 2 * np.pi)
    ...
    if args.render == 'image' and chosen == False:
        (path / 'images').mkdir(exist_ok=True)
        imgpath = path / f"images/image_{idx:03d}.png"
        
        if os.path.exists(path / f"images/image_{idx:03d}.png"):
            imgpath = path / f"images/image_{idx:03d}_2.png"
        else:
            imgpath = path / f"images/image_{idx:03d}.png"
    ...
    elif args.render == 'image' and chosen == True: ## If chosen is True, then render two angles of object
        (path / 'images').mkdir(exist_ok=True)
        imgpath = path / f"images/image_{idx:03d}.png"
        scene.render.filepath = str(imgpath)
        bpy.ops.render.render(write_still=True)
        build_scene(path, idx, factory_name, args, False) ## Recursively call build_scene to render the second angle of the object
...
def setup_camera(args, cam_dist, cam_angle):
    ...
    camera.parent.rotation_euler = np.deg2rad(cam_angle)
...
def make_grid(args, path, n):
    ...
    # Ensure grid dimensions match the sum of the dimensions of the images placed in it
    sz_x = args.grid_size_x
    sz_y = args.grid_size_y
    grid_width = sz_x * img_width
    grid_height = sz_y * img_height
    img = Image.new('RGBA', (grid_width, grid_height))

    for idx, file in enumerate(files):
        with Image.open(file) as i:
            img_x = (idx % sz_x) * img_width
            img_y = (idx // sz_x) * img_height
            img.paste(i, (img_x, img_y))
...
def main(args):
    ...
    for fac in factories:
        ...
        # Generate assets with the seed and seed + 1.
        rand = random.randint(0, 2**32 - 2)
        seeds = [args.seed, args.seed + 1] if args.seed >= 0 else [rand, rand + 1]
        chosen = bool(np.random.choice([True, False]))
        for seed in seeds:
            try:
                build_scene(fac_path, seed, fac, args, chosen)
                chosen = not chosen
            except Exception as e:
                print(e)
                continue
        ...
        if args.render == 'image':
            make_grid(args, fac_path, len(seeds) * 3)
...
def make_args():
    ...
    parser.add_argument('--cam_dist_min', type=float, default=20.0, help="Minimum camera distance")
    parser.add_argument('--cam_dist_max', type=float, default=30.0, help="Maximum camera distance")
    parser.add_argument('--cam_angle_min', type=float, nargs=3, default=(-180, -180, -180), help="Minimum camera angle (X, Y, Z) in degrees")
    parser.add_argument('--cam_angle_max', type=float, nargs=3, default=(180, 180, 180), help="Maximum camera angle (X, Y, Z) in degrees")

    parser.add_argument('--grid_size_x', type=int, default=3, help="Number of columns in the image grid")
    parser.add_argument('--grid_size_y', type=int, default=1, help="Number of rows in the image grid")
